...
class MultiLayer_NeuralNet():
	...
	def create_a_h(self):
		...
		self.dh = [[] for i in range(1+self.num_hidden)]
	...
	def solve_network(self,X,y):
		...
		while t<max_epoch:
			...
			self.feed_forward(X[step * self.batch_size : (1+step) * self.batch_size, :], y)
  ...
  #To compute the derivatives wrt weights and biases using chain rule.
	def backprop(self, X, y, lbda):
    	...
    	#
		self.da[0] = np.multiply(self.dh[1], self.d_active[self.activation](self.h[0]))
	...
	def draw_neural_net(self, ax, left, right, bottom, top, layer_sizes, grad, weights, preactivations, values, layer_num, neuron_num, curr_y=None):
	    '''
	    ...
	    for n, layer_size in enumerate(layer_sizes):
	        ...
	        for m in xrange(layer_size):
	            ...
	            # lab = "{}".format(round(values[n][m], 2))
	            # ax_button = ax1.axes((n*h_spacing + left, layer_top - m*v_spacing, 0.09, 0.09), facecolor='w')
	            # bnext = Button(ax_button, label=str(lab), color='cyan')
	            fc = "white"
	            if n == layer_num and m == neuron_num:
	            	fc = "green"
	            if layer_num == -1 and n == len(layer_sizes)-1:
	            	fc = "cyan"
	            ...
	            if layer_num == -1 and n == len(layer_sizes)-1: # loss computation
	...
	def plot_error(self):
	    ax3.clear()
	    # rect = patches.Rectangle((14,17),10,1,color='w',fill=False)
	    # error_bar = patches.Rectangle((14, 17), 10 * self.Loss_arr[-1]/100, 1, color='red')
	    # ax3.add_patch(rect)
	    # ax3.add_patch(error_bar)
	...
	def Animate(self, i):
		...
		for l in layer_val[1:]:
			for num in range(0,l):
				self.draw_neural_net(fig.gca(), 0, 0.9, 0, 0.9, layer_val, self.dw, self.weights, preactivations, values, l_num, num)
				plt.pause(2)
		...
		# loss computation
		self.draw_neural_net(fig.gca(), 0, 0.9, 0, 0.9, layer_val, self.dw, self.weights, preactivations, values, -1, -1, curr_y)
		plt.pause(2)
		fig.savefig('Plots/img_' + str(img_count))
		img_count+=1
